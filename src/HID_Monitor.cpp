                                 ...
                                 size_t nameSize)
{
    ...
    api.lib_AmfiProt_EncodeFrame(&txFrame, &payload, sizeof(payload),
        libAmfiProt_PayloadType_Common, 0,
        lib_AmfiProt_destination_Broadcast, lib_AmfiProt_packetType_NoAck);
    ...
    api.lib_AmfiProt_EncodeFrame(&txFrame, &payload, sizeof(payload),
        libAmfiProt_PayloadType_Common, 0,
        deviceIdOut, lib_AmfiProt_packetType_NoAck);
...
HIDMonitor::~HIDMonitor() { shutdown(); }
...
bool HIDMonitor::init()
{
    ...
    if (hid_init() != 0) { std::printf("hid_init() failed\n"); return false; }
    _initialized = true;
    syncDevices();
    return true;
}
...
bool HIDMonitor::shutdown()
{
    ...
    std::lock_guard<std::mutex> lock(_mutex);
#endif
    for (auto& s : _sensors) if (s._dev_handle) { hid_close(s._dev_handle); s._dev_handle = nullptr; }
    for (auto& s : _sources) if (s._dev_handle) { hid_close(s._dev_handle); s._dev_handle = nullptr; }
    _sensors.clear();
    _sources.clear();
    if (_initialized) { hid_exit(); _initialized = false; }
    return true;
}
...
void HIDMonitor::scanForPid(uint16_t pid)
{
    ...
    for (const hid_device_info* info = list; info; info = info->next)
    {
        ...
        auto alreadyOpen = [&](const auto& d){ return isSameDevice(d._dev_handle, info); };
        if (pid == PID_Sensor && std::any_of(_sensors.begin(), _sensors.end(), alreadyOpen)) continue;
        if (pid == PID_Source && std::any_of(_sources.begin(), _sources.end(), alreadyOpen)) continue;
        ...
        if (pid == PID_Sensor)
        {
            ...
            sensor.active      = true;
            if (!probeSensorIdentity(sensor)) { hid_close(handle); continue; }
        ...
        else
        {
            ...
            source.active      = true;
            if (!probeSourceIdentity(source)) { hid_close(handle); continue; }
...
bool HIDMonitor::probeSensorIdentity(AMFITRACK_Sensor& sensor)
{
    ...
    auto writeFn = [this](hid_device* h, const void* d, size_t l){ return hidWrite(h, d, l); };
    auto readFn  = [this](hid_device* h, void* d, int t)          { return hidReadTimeout(h, d, t); };
...
{
    if (!_cb.txPoll || !_cb.txDone) return;

    size_t  queueIdx = 0, dataLen = 0;
    uint8_t txId = 0;
    void*   txData = nullptr;

    if (!_cb.txPoll(queueIdx, dataLen, txId, txData)) return;

    bool sent = false;
    if (txId == 255)
    {
        for (auto& s : _sensors) if (s._dev_handle && hidWrite(s._dev_handle, txData, dataLen) >= 0) sent = true;
        for (auto& s : _sources) if (s._dev_handle && hidWrite(s._dev_handle, txData, dataLen) >= 0) sent = true;
    }
    ...
    if (sent) _cb.txDone((uint8_t)queueIdx);
}

void HIDMonitor::drainRx()
{
    if (!_cb.rxPush) return;
    ...
    for (auto& s : _sensors) readFrom(s._dev_handle);
    for (auto& s : _sources) readFrom(s._dev_handle);
}

// ─────────────────────────────────────────────────────────────────────────────
hid_device* HIDMonitor::findHandleByTxId(uint8_t txId)
{
    for (auto& s : _sensors) if (s.deviceId == txId) return s._dev_handle;
    for (auto& s : _sources) if (s.deviceId == txId) return s._dev_handle;
    return nullptr;
}
